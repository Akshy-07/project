...
def init_db():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS fees (student_id INTEGER PRIMARY KEY, status TEXT, tuition_fee REAL, exam_fee REAL, other_fee REAL, total_fee REAL, paid_amount REAL, due_amount REAL)''')
...
def admin_dashboard():
    ...
    try:
        ...
        return "An error occurred while loading the dashboard. Please try again."
    finally:
        conn.close()
...
def add_staff():
    ...
    if not all([name, dept, subject]):
        return redirect(url_for('admin_dashboard'))
        
    conn = get_db_connection()
    try:
        conn.execute("INSERT INTO staff (name, dept, subject) VALUES (?, ?, ?)", (name, dept, subject))
        conn.commit()
    except Exception as e: 
...
def manage_fees():
    ...
    try:
        ...
        logging.error(f"Error managing fees: {e}")
    finally:
        conn.close()
    return redirect(url_for('admin_dashboard'))
...
def staff_dashboard():
    ...
    try:
        ...
        return render_template('staff.html', requests=requests_list, announcements=announcements)
    except Exception as e:
        logging.error(f"Error loading staff dashboard: {e}")
        return "An error occurred while loading the staff dashboard."
    finally:
        conn.close()
...
def add_student_api():
    ...
    try:
        ...
        logging.error(f"Error adding student api: {e}")
        return jsonify({'success': False, 'message': 'Database error'}), 500
    finally:
        conn.close()
...
def add_marks():
    ...
    if not reg or not subject:
        return jsonify({'success': False, 'message': 'Missing identifying fields'}), 400
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'Invalid numerical values passed for marks.'})
    ...
    try:
        ...
        logging.error(f"Error adding marks api: {e}")
        return jsonify({'success': False, 'message': 'Database error'}), 500
    finally:
        conn.close()
...
def update_attendance_api():
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'Invalid percentage logic submitted.'})
    ...
    try:
        ...
        logging.error(f"Error updating attendance api: {e}")
        return jsonify({'success': False, 'message': 'Database error'}), 500
    finally:
        conn.close()
...
def student_dashboard():
    ...
    try:
        ...
        return render_template('student.html', student=student_tuple, attendance=attendance, marks=marks, 
                               fees=fees, timetable=timetable, leave_requests=leave_requests, 
                               internal_marks=internal_marks, announcements=announcements)
    except Exception as e:
        logging.error(f"Error loading student dashboard: {e}")
        return "An error occurred fetching dashboard modules."
    finally:
        conn.close()
